...
export function registerCollectorRoutes(app: Express) {
  ...
  app.get('/api/collector/places', async (req, res) => {
    try {
      ...
      if (country) {
        ...
        filteredPlaces = filteredPlaces.filter(place => 
          place.address.toLowerCase().includes(countryTerm)
        );
      }
  ...
  app.get('/api/collector/countries/:country/places', async (req, res) => {
    try {
      ...
      const countryPlaces = FALLBACK_PLACES.filter(place => 
        place.address.toLowerCase().includes(country.toLowerCase())
      );
      
      const startIndex = parseInt(offset as string) || 0;
      const endIndex = startIndex + (parseInt(limit as string) || 20);
