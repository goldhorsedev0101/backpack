      ...
      `, (err, rows) => {
        ...
        for (const place of places) {
            try {
                ...
                const isAccommodation = placeTypes.some(type => type.includes('hostel') ||
                    type.includes('lodging') ||
                    type.includes('hotel') ||
                    type.includes('accommodation'));
                const isAttraction = placeTypes.some(type => type.includes('tourist_attraction') ||
                    type.includes('museum') ||
                    type.includes('park') ||
                    type.includes('landmark'));
                const isRestaurant = placeTypes.some(type => type.includes('restaurant') ||
                    type.includes('food') ||
                    type.includes('cafe') ||
                    type.includes('bar'));
...
function determineAccommodationType(types) {
    const typeStr = types.join(' ').toLowerCase();
    if (typeStr.includes('hostel'))
        return 'hostel';
    if (typeStr.includes('hotel'))
        return 'hotel';
    if (typeStr.includes('lodge'))
        return 'lodge';
    if (typeStr.includes('guesthouse'))
        return 'guesthouse';
