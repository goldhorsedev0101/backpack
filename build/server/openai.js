...
export async function generateItinerary(destination, duration, budget, preferences) {
    try {
...
export async function analyzeBudget(expenses, totalBudget) {
    try {
...
export async function generateRecommendations(destination, reviews) {
    try {
...
export async function conversationalTripAssistant(message, context = {}) {
    try {
...
export async function generateConversationalSuggestions(chatHistory, previousSuggestions = []) {
    try {
...
export async function enrichSuggestionsWithRealPlaces(suggestions) {
    try {
        console.log('Enriching suggestions with real places...');
        // Add timeout wrapper for the entire enrichment process
        const enrichmentPromise = Promise.all(suggestions.map(async (suggestion) => {
            const realPlaces = [];
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Enrichment timeout')), 15000));
