...
import { insertTripSchema, insertReviewSchema, insertExpenseSchema, insertConnectionSchema, insertPlaceReviewSchema, insertReviewVoteSchema, insertChatRoomSchema, insertTravelBuddyPostSchema, insertTravelBuddyApplicationSchema, insertLocationReviewSchema, } from "@shared/schema";
import { generateTravelSuggestions, analyzeBudget, generateRecommendations, chatAssistant, conversationalTripAssistant, generateConversationalSuggestions, enrichSuggestionsWithRealPlaces } from "./openai";
...
export async function registerRoutes(app) {
    ...
    app.post('/api/user/registry', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims?.sub || req.user.id;
    ...
    app.get('/api/expenses/user', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
    ...
    app.post('/api/data/seed', isAuthenticated, async (req, res) => {
        try {
    ...
    app.post('/api/places/import', isAuthenticated, async (req, res) => {
        try {
    ...
    app.get('/api/places/attractions/:location', async (req, res) => {
        try {
            ...
            const results = await googlePlaces.searchPlaces(location, 'tourist_attraction', location);
    ...
    app.get('/api/tripadvisor/search', isAuthenticated, async (req, res) => {
    ...
    app.get('/api/tripadvisor/location/:locationId', isAuthenticated, async (req, res) => {
    ...
    app.get('/api/tripadvisor/reviews/:locationId', isAuthenticated, async (req, res) => {
    ...
    app.post('/api/ai/generate-trip', isAuthenticated, async (req, res) => {
        try {
    ...
    app.post('/api/ai/budget-analysis', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
    ...
    app.post('/api/ai/conversational-suggestions', isAuthenticated, async (req, res) => {
        try {
    ...
    app.post('/api/ai/chat-legacy', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
    ...
    app.get('/api/weather/:destination', async (req, res) => {
        try {
            ...
            try {
                ...
                weatherData = await weatherService.getCurrentWeather(destination, country);
                if (!weatherData) {
    ...
    app.get('/api/weather/:destination/recommendations', async (req, res) => {
        try {
            ...
            try {
                ...
                const weatherData = await weatherService.getCurrentWeather(destination, country);
    ...
    app.get('/api/activities/:destination', async (req, res) => {
        try {
            ...
            const activities = [
                ...
                (!duration || activity.duration.includes(duration)));
            res.json(activities);
        }
        catch (error) {
    ...
    app.get('/api/restaurants/:destination', async (req, res) => {
        try {
            ...
            ].filter(restaurant => (!cuisine || restaurant.cuisine === cuisine) &&
    ...
    app.get('/api/deals', async (req, res) => {
        try {
            ...
            ].filter(deal => (!destination || deal.destination.toLowerCase().includes(destination.toString().toLowerCase())) &&
        ...
        try {
            ...
            const analytics = {
                trips: {
                    ...
                    completed: userTrips.filter((trip) => new Date(trip.endDate) < new Date()).length,
                    upcoming: userTrips.filter((trip) => new Date(trip.startDate) > new Date()).length,
                    countries: Array.from(new Set(userTrips.map((trip) => trip.destinations?.[0]?.country).filter(Boolean))).length
                },
                expenses: {
                    ...
                    topCategory: userExpenses.reduce((acc, expense) => {
                        acc[expense.category] = (acc[expense.category] || 0) + parseFloat(expense.amount);
                        return acc;
                    }, {}),
                    avgPerTrip: userTrips.length > 0 ? userExpenses.reduce((sum, expense) => sum + parseFloat(expense.amount), 0) / userTrips.length : 0
                },
                destinations: {
                    visited: Array.from(new Set(userTrips.map((trip) => trip.destinations?.[0]?.name).filter(Boolean))),
    ...
    app.get('/api/search', async (req, res) => {
        try {
            ...
            const results = {
                ...
                ].filter(item => !type || type === 'destinations').filter(item => item.name.toLowerCase().includes(query) || item.country.toLowerCase().includes(query)),
                activities: [
                ...
                ].filter(item => !type || type === 'activities').filter(item => item.name.toLowerCase().includes(query) || item.location.toLowerCase().includes(query)),
                restaurants: [
                ...
                ].filter(item => !type || type === 'restaurants').filter(item => item.name.toLowerCase().includes(query) || item.location.toLowerCase().includes(query))
            };
            const allResults = [
    ...
    */
    ...
    app.get('/api/accommodations', async (req, res) => {
        try {
            ...
            let accommodations = [];
            try {
                accommodations = await storage.getAccommodations(destinationId ? parseInt(destinationId) : undefined);
            }
            catch (dbError) {
    ...
    app.get('/api/attractions', async (req, res) => {
        try {
            ...
            let attractions = [];
            try {
                attractions = await storage.getAttractions(destinationId ? parseInt(destinationId) : undefined);
            }
            catch (dbError) {
                console.log('Database error, using mock data:', dbError);
    ...
    app.get('/api/ta-restaurants', async (req, res) => {
        try {
            ...
            let restaurants = [];
            try {
                restaurants = await storage.getRestaurants(destinationId ? parseInt(destinationId) : undefined);
            }
            catch (dbError) {
                console.log('Database error, using mock data:', dbError);
    ...
    app.post('/api/location-reviews', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
    ...
    app.get('/api/place-reviews/user', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user?.claims?.sub || req.user?.id;
    ...
    app.patch('/api/place-reviews/:id', isAuthenticated, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            const userId = req.user?.claims?.sub || req.user?.id;
    ...
    app.post('/api/review-votes', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user?.claims?.sub || req.user?.id;
    ...
    app.get('/api/chat-rooms', async (req, res) => {
        try {
            ...
            if (search) {
                const rooms = await storage.searchChatRooms(search, {
                    type: type,
                    destination: destination
                });
    ...
    app.post('/api/travel-buddy-posts', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user?.claims?.sub || req.user?.id;
    ...
    app.get('/api/travel-buddy-posts/user', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user?.claims?.sub || req.user?.id;
    ...
    app.post('/api/travel-buddy-applications', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user?.claims?.sub || req.user?.id;
    ...
    app.get('/api/travel-buddy-posts/:id/applications', isAuthenticated, async (req, res) => {
        try {
    ...
    app.patch('/api/travel-buddy-applications/:id', isAuthenticated, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            const { status } = req.body;
